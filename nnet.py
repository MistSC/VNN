...
class GaussianSampleLayer(object):
	def __init__(self, mu, log_sigma, n_in, batch_size):
		...
		#epsilon = np.asarray(srng.normal(size=(batch_size,n_in)), dtype=theano.config.floatX)
		#self.output = mu + T.exp(0.5*log_sigma) * epsilon
		self.output = (mu + T.exp(log_sigma) * epsilon)
...
class SoftmaxLayer(object):
	def __init__(self, rng, input, n_in, n_out, name='', W=None, b=None):
		...
		#self.output = T.argmax(self.p_y_given_x, axis=1, keepdims=False)
		# end-snippet-1
...
'''		  
class Stacked_NN_0L:
	def __init__(self, rng, input, struct, name=''):
		if len(struct.layer_dim) != 2:
			print('used wrong NN Block size')			 
...
class Stacked_NN_2L:
	def __init__(self, rng, input, struct, name=''):
		...
		self.OL = NNLayer(
			...
			n_in=struct.layer_dim[2],
			n_out=struct.layer_dim[3],
			activation=struct.activation[2],
			name=name+'_OL'
		)
		
		self.output = self.OL.output
		self.params = self.HL_1.params + self.HL_2.params + self.OL.params
